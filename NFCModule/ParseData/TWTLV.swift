...
open class TWTLV : NSObject {
	...
	public init(data:[UInt8], offset:inout Int) throws {
		...
		if let tagId = TWTLV.getTagId(data, offset: &offset){
			...
			self.length = length
            print("Uint64 \(UInt64(data.count-offset))")
    ...
    open func printableTlv(_ level:Int = 1) -> (value: String, arrayValue: [String]){
		var tlvStr = ""
		...
		if(length > 0x00){
			...
			if(constructed){
				...
				for child in children{
                    ...
                    tlvStr += child.printableTlv(level+1).0
				}
			...
			if(!tlvStr.hasSuffix(":\n")){
                ...
                if (level >= 3) {
                    let _tlvStr = tlvStr.split(separator: ":")
                    arrayBytes.append(String(_tlvStr[1]))
                }
				tlvStr += ":\n"
        ...
        let _ = arrayBytes.filter({!$0.isEmpty})
		return (tlvStr, arrayBytes)
	}
