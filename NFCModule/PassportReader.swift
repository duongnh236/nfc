...
extension PassportReader : NFCTagReaderSessionDelegate {
    ...
    public func tagReaderSession(_ session: NFCTagReaderSession, didInvalidateWithError error: Error) {
        ...
        Log.debug( "tagReaderSessio n:didInvalidateWithError - \(error.localizedDescription)" )
        ...
        if let readerError = error as? NFCReaderError, readerError.code == NFCReaderError.readerSessionInvalidationErrorUserCanceled
            ...
            if let readerError = error as? NFCReaderError {
                ...
                case NFCReaderError.readerSessionInvalidationErrorSessionTimeout:
                    ...
                    let errorMessage = NFCViewDisplayMessage.error(.TimeOut)
                    self.invalidateSession(errorMessage: errorMessage, error: NFCPassportReaderError.TimeOut)
...
extension PassportReader {
    ...
    func startReading(tagReader : TagReader) async throws -> NFCPassportModel {
        ...
        if let dg13 = self.passport.getDataGroup(.DG13) as? DataGroup13 {
            print(dg13)
        }
    ...
    func readDataGroups( tagReader: TagReader ) async throws {
        ...
        if DGsToRead.contains( .DG13 ) {
            DGsToRead.removeAll { $0 == .DG13 }
            
            if !skipCA {
                // Do Chip Authentication
                if let dg13 = try await readDataGroup(tagReader:tagReader, dgId:.DG13) as? DataGroup13 {
                    self.passport.addDataGroup( .DG13, dataGroup:dg13 )
    ...
    func readDataGroup( tagReader : TagReader, dgId : DataGroupId ) async throws -> DataGroup?  {
        ...
        repeat {
            do {
                ...
                if (dgId == DataGroupId.getIDFromName(name: "DG13")) {
